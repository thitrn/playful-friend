...
def play_level(level_name, rounds, wins_needed):
    level_wins = 0
    rounds_played = 0
    ...
    for title, prompt, sequence, round_time, threshold in rounds:
        rounds_played += 1
        ...
        remaining_rounds = 3 - rounds_played
        if level_wins + remaining_rounds < wins_needed:
...
def main():
    try:
        ...
        passed_level_1 = play_level("Level 1. Easy.", level1_rounds, wins_needed=2)

        if passed_level_1:
            say("Now it is time for level 2. Get ready!")
            sleep(1)
            passed_level_2 = play_level("Level 2. Intermediate.", level2_rounds, wins_needed=2)
        else:
